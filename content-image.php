...
<article id="post-<?php the_ID(); ?>" <?php post_class( $ratio) ; ?>>
	...
	<aside>
		<ul class="image-meta">
			...
			<li class="date_taken"><span class="label opt"><?php _e( 'Taken on ' , 'fumseck' ); ?></span><time datetime="<?php echo esc_attr( date( 'c', $exif_date_taken ) ); ?>"><?php echo date_i18n( __("F j, Y", 'fumseck'), $exif_date_taken ) ; ?></time></li>
					<?php };?>
			<?php };?>
			
			<?php if ( $batbelt_location = get_the_term_list( get_the_ID(), 'batbelt_locations', '', __( ', ', 'fumseck' ) ) ) {?>
			<li class="location"><span class="label opt"><?php _ex( 'in ' , 'in location', 'fumseck' ); ?></span><?php printf( $batbelt_location ); ?></li>
			<?php };?>
			
			<?php if ( $batbelt_event = get_field( '_event', get_the_ID(), true ) ) {?>
			<li class="event"><span class="label"><?php _e( 'during ' , 'fumseck' ); ?></span><?php fumseck_linked_title( $batbelt_event ); ?></li>
			<?php };?>
			
			<?php if ( $featured_image_exif ) {
				if ( $exif_aperture = $featured_image_exif['image_meta']['aperture'] ) {?>
			<li class="aperture"><span class="label opt"><?php _e( 'Aperture: ' , 'fumseck' ); ?></span><?php echo '<i>ƒ</i>/' . sanitize_text_field( $exif_aperture ) ; ?></li>
				<?php };?>
			
				<?php if ( $exif_focal_length = $featured_image_exif['image_meta']['focal_length'] ) {?>
			<li class="focal_length"><span class="label opt"><?php _e( 'Focal length: ' , 'fumseck' ); ?></span><?php echo sanitize_text_field( $exif_focal_length ) . ' mm' ; ?></li>
				<?php };?>
			
				<?php if ( $exif_exposure = $featured_image_exif['image_meta']['shutter_speed'] ) {?>
			<li class="exposure"><span class="label opt"><?php _e( 'Exposure time: ' , 'fumseck' ); ?></span><?php echo sanitize_text_field( $exif_exposure ) . ' s' ; ?></li>
				<?php };?>
			<?php }; # end exif?>
	...
	<aside>
		<ul class="image-pub-meta">
			...
			<li class="people"><span class="label"><?php _e( 'People: ' , 'fumseck' ); ?></span><?php echo $batbelt_people_list ; ?></li>
			<?php }; ?>
