...
<article id="post-<?php the_ID(); ?>" <?php post_class('regular'); ?>>
	...
	<header>
		...
		<ul class="meta1">
			...
			<li class="categories"><?php echo $categories_list ; ?></li>
			<?php }; ?>
				
			<?php if ( $start_date = get_field( '_start_date', get_the_ID(), true ) ) { ?>
			<li class="start_date"><span class="label"><?php _e( 'Started ' , 'fumseck' ); ?></span><time datetime="<?php echo esc_attr( $start_date ) ; ?>"><?php echo date_i18n( __('F j, Y', 'fumseck'), strtotime( $start_date ) ); ?></time></li>
			<?php }; ?>
		
			<?php if ( $end_date = get_field( '_end_date', get_the_ID(), true ) ) { ?>
			<li class="end_date"><span class="label"><?php _e( 'Completed: ' , 'fumseck' ); ?></span><time datetime="<?php echo esc_attr( $end_date ) ; ?>"><?php echo date_i18n( __('F j, Y', 'fumseck'), strtotime( $end_date ) ); ?></time></li>
			<?php }elseif ( $status = get_field( '_status', get_the_ID(), true ) ) { ?>
			<li class="status"><span class="label"><?php _e( 'Status: ' , 'fumseck' ); ?></span><?php echo $status; ?></time></li>
			<?php }; ?>
	...
	<aside>
		<ul class="meta2">
			...
			<li class="roles"><span class="label"><?php _e( 'Roles: ' , 'fumseck' ); ?></span><?php echo $batbelt_roles_list ; ?></li>
			<?php }; ?>
			...
			<li class="projects"><span class="label"><?php _e( 'Part of: ' , 'fumseck' ); ?></span><?php fumseck_linked_title( $batbelt_project ); ?></li>
			<?php }; ?>
